...
import com.corhuila.BackendCoursys.Dto.ApiResponseDto;
import com.corhuila.BackendCoursys.Entity.ABaseEntity;
import com.corhuila.BackendCoursys.IService.IBaseService;
...
public abstract class ABaseController<T extends ABaseEntity, S extends IBaseService<T>> {
    ...
    public ResponseEntity<ApiResponseDto<T>> delete(@PathVariable Long id) {
        try {
            service.setStatus(id, false); // Eliminación lógica
            return ResponseEntity.ok(new ApiResponseDto<>("Registro eliminado lógicamente", null, true));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(new ApiResponseDto<>(e.getMessage(), null, false));
        }
    }
